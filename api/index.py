...
# Essencial para o Vercel e o HTML conversarem sem bloqueio
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
# --- API DE DADOS (Substitui as rotas de HTML) ---
...
async def gerar_insights():
    res = supabase.table('estoque').select("*").execute()
    insights = [f"Opa! {i['nome']} tá acabando!" for i in res.data if i['quantidade'] < 5]
    return {"avisos": insights or ["Estoque equilibrado no Mercadão!"]}
