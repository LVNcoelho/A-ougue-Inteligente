...
# 1. CONFIGURAÇÃO DE VARIÁVEIS (Uma única vez)
...
async def listar_vendas():
    res = supabase.table('historico_pedidos').select("*").order('id', desc=True).limit(5).execute()
    return res.data
...
async def registrar_venda(nome_cliente: str = Form(...), whatsapp: str = Form(...), carne_id: int = Form(...), peso_vendido: float = Form(...)):
    ...
    supabase.table('historico_pedidos').insert({
        ...
        "itens_comprados": f"{peso_vendido}kg de {carne.data['nome']}", 
